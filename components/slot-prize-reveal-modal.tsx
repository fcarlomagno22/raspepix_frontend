...
export function SlotPrizeRevealModal({ isOpen, onClose, onPrizeRevealed }: SlotPrizeRevealModalProps) {
  ...
  useEffect(() => {
    if (isOpen && !hasReported.current) {
      ...
      const formattedPrize = randomPrize.toFixed(2)
      setPrizeAmount(formattedPrize.replace(".", ","))
      onPrizeRevealed(randomPrize)
    ...
    <Dialog open={isOpen} onOpenChange={onClose}>
      ...
      >
        ...
        <div className="p-4 text-center">
          ...
          <p className="text-md text-gray-400">Seu saldo foi atualizado!</p>
        </div>
      </DialogContent>
    </Dialog>
  )
